...
def peak_memory_analysis(mem_snapshot_filepath, mem_snapshot_csv_dir, memory_allocated, num_mb):
    """
    Key and Entry Function for memory analysis. 
    """
    ...
    for i in range(len(peaks) - 1):
        ...
        csv_new_path = os.path.join(mem_snapshot_csv_dir, f"real_new_memory_peak_{i}_{i+1}.csv")
        ...
        csv_gone_path = os.path.join(mem_snapshot_csv_dir, f"real_gone_memory_peak_{i}_{i+1}.csv")
    ...
    df_new = pd.DataFrame(data_new)
    csv_new_path = os.path.join(mem_snapshot_csv_dir, f"real_new_memory_peak_{NUM_MB-1}_{2*NUM_MB-1}.csv")
    df_new.to_csv(csv_new_path, index=False)
    ...
    df_gone = pd.DataFrame(data_gone)
    csv_gone_path = os.path.join(mem_snapshot_csv_dir, f"real_gone_memory_peak_{NUM_MB-1}_{2*NUM_MB-1}.csv")
    df_gone.to_csv(csv_gone_path, index=False)
